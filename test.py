    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        for box_pr, target in zip([all_boxes], [target[0]]):
            ...
            for k in range(num_gts):
                ...
                limit = ([False, True, True, True, True, True, True, True, True])
                corners2D_gt = np.array(np.reshape(box_gt[:18], [9, 2]), dtype='float32')
                corners2D_pr = np.array(np.reshape(box_pr[:18], [9, 2]), dtype='float32')
                corners2D_gt[:, 0] = corners2D_gt[:, 0] * im_width
                corners2D_gt[:, 1] = corners2D_gt[:, 1] * im_height
                # corners2D_gt = corners2D_gt[limit]
                print(corners2D_gt)
                corners2D_pr[:, 0] = corners2D_pr[:, 0] * im_width
                corners2D_pr[:, 1] = corners2D_pr[:, 1] * im_height
                print(corners2D_pr)
                # corners2D_pr = corners2D_pr[limit]
                ...
                if visualize:
                    ...
                    for edge in edges_corners:
                        plt.plot(corners2D_gt[edge, 0], corners2D_gt[edge, 1], color='g', linewidth=3.0)
